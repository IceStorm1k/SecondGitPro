...
/* app.listen(PORT, HOST, () => {
  console.log(`Server is running on http://${HOST}:${PORT}`); // Сообщение о запуске сервера
}); */
...
const startServer = async () => {
  try {
    ...
    const chalk = await import('chalk');
    console.log(chalk.default.red(`Error: ${error.message}`)); // Сообщение об ошибке красным цветом
  }
};
